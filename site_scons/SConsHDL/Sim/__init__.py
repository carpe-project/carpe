...
def Kconfig(kconfig, env):
    tool_valuelist = kconfig.ValueList('hdl.sim.tool',
                                       prompt = '"Simulation Toolchain Selection"',
                                       optional = True,
                                       )
    for tool in tools.keys():
        tool_valuelist.Value('hdl.sim.tool.%s' % tools[tool].name,
                             prompt = '"%s"' % tools[tool].Name,
                             value = tool,
                             )
    for tool in tools.keys():
        tool_menu = kconfig.Menu('%s Options' % tools[tool].Name,
                                 depends_on = 'hdl.sim.tool.%s' % tools[tool].name,
                                 )
        tools[tool].Kconfig(tool_menu, env)

def Tool(kconfig):
    try:
        return tools[kconfig['hdl.sim.tool']]
    except KeyError:
        return None
